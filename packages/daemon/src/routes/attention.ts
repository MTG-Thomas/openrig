  ...
  app.get("/", c => {
    ...
    try {
      ...
      for (const q of [...current, ...(detailOnly ? [detailOnly] : [])]) {
        ...
        const item: AttentionItem = { id: `queue:${q.qitemId}`, kind: "action", summary: q.summary || q.body.trim().split(/\r?\n/).find(Boolean) || "Request summary unavailable", urgency: q.priority, unblocks, at: q.tsUpdated, scope: p ? `project ${p.id}` : "instance · project unknown", project: p ? { id: p.id, root: p.root } : null, source: `/api/queue/${encodeURIComponent(q.qitemId)}` };
        add(item, () => {
          const mode = posture?.resolve({ qitemId: q.qitemId });
          return [`State: ${q.state}`, `To: ${q.destinationSession}`, `From: ${q.sourceSession}`, `Blocked on: ${q.blockedOn ?? "none"}`, `Posture: ${mode?.posture ?? "unknown"} · ${mode?.reason ?? "posture source unavailable"}`, `Evidence reference: ${q.evidenceRef ?? "none recorded"}`, "Request:", q.body, "Queue history (read/delivery is not approval):", ...queue.listTransitions(q.qitemId).map(t => JSON.stringify(t))];
        }, () => {
