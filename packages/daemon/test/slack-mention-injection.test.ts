...
describe("R2 B1 — queue-controlled content must be structurally INERT in posted bytes", () => {
  ...
  it("F-B1r NEGATIVE: a HOSTILE caller-supplied qitemId (<!channel>) emits zero active control syntax on any posted surface — the internal id stays out of human prose while reconciliation remains decision-scoped", async () => {
